...
class GetRelativeCoordinateError(TestStep):
    ...
    def process_group(self, param, orig_quads, det_quads):
        ...
        if det_quads.count(None) > (len(det_quads) - 5):
            ...
            error = [quad.get_error_rel_avg(q, det) for (q, det) in zip(orig_quads, det_quads) if det is not None]
    ...
    def save_results(self):
        ...
        bi.output_file("out/plots/relative_coordinate_error.html", title="Relative error")
        bi.save(self.figure)
